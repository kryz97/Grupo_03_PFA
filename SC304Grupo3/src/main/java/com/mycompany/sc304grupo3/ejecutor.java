...
public class ejecutor {
    ...
    public void BalanceGrafo() {
        BalanceGrafo balanceGrafo = new BalanceGrafo(7);

        // Información para el balance
        balanceGrafo.agregarArista(0, 1, 15.50 - 20.00);  // Lomo Saltado
        balanceGrafo.agregarArista(1, 2, 18.75 - 25.00);  // Risotto de Mariscos
        balanceGrafo.agregarArista(2, 3, 8.50 - 12.00);   // Tacos Al Pastor
        balanceGrafo.agregarArista(3, 4, 6.25 - 10.00);   // Sopa de Cebolla
        balanceGrafo.agregarArista(4, 5, 22.00 - 30.00);  // Filete Mignon
        balanceGrafo.agregarArista(5, 6, 20.75 - 28.00);  // Paella Valenciana
        balanceGrafo.agregarArista(6, 0, 7.00 - 11.00);   // Ensalada Caprese

        balanceGrafo.imprimirMatrizAdyacencia();

        // Se realiza el recorrido en amplitud desde el nodo 0 (Lomo Saltado)
        balanceGrafo.recorridoAmplitud(0);
        // Calcula y muestra las ganancias diarias
        balanceGrafo.balanceDiario();
    }
